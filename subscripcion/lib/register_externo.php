...
$name=mysqli_query($db_connection, "SELECT * FROM ".$db_table_name." WHERE Nombre = '".$subs_name."'");
$email=mysqli_query($db_connection, "SELECT * FROM ".$db_table_name." WHERE Email = '".$subs_email."'");



if (mysqli_num_rows($name)>0 || mysqli_num_rows($email)>0)
{
   while ($data = mysqli_fetch_array($name)) { 
      /* echo '<script language="javascript">alert("`' . $data["id"] . '`");</script>'; */
      $LocalName=$data["Nombre"];
      $LocalEmail=$data["Email"];
   }
   
   echo '<script language="javascript">alert("El usuario ' . $subs_name . ' con el correo: '. $subs_email .' ya esta registrado.");
